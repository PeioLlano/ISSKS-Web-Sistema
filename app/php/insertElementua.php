<?php
  ...
  $ordutegiKonprobaketa = mysqli_query($conn, "SELECT COUNT(*) FROM `elementua` WHERE `kirola`=$kirola AND `data`=$data AND `ordutegia`=$ordutegia; ");
  if(empty($ordutegiKonprobaketa)){
    ...
    $ilara = mysqli_num_rows($elemKonprobaketa);
    if($ilara < 5){
